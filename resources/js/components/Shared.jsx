    ...
    return (
        <div className="flex items-center justify-end">
            <a target="_blank" href={info.facebook}>
                ...
                <svg
                    className="fill-custom-dark hover:fill-custom-blue transition-all"
                ...
                >
                    <path
                        id="facebook-rect"
                        d="M4.592,0A4.582,4.582,0,0,0,0,4.592V20.558A4.582,4.582,0,0,0,4.592,25.15h8.653V15.318h-2.6v-3.54h2.6V8.754c0-2.376,1.536-4.558,5.075-4.558a21.442,21.442,0,0,1,2.492.138L20.729,7.64s-1.081-.01-2.259-.01c-1.276,0-1.48.588-1.48,1.564v2.585h3.842l-.168,3.54H16.99v9.833h3.569a4.582,4.582,0,0,0,4.592-4.592V4.592A4.582,4.582,0,0,0,20.559,0Z"
                    />
                </svg>
            </a>
...
export const DirectionBtn = ({ link, text, back }) => {
    return (
        ...
        >
            ...
            <svg
                className={`${back ? "rotate-180" : ""} ml-2`}
                xmlns="http://www.w3.org/2000/svg"
                width="47.049"
                height="7.518"
                viewBox="0 0 47.049 7.518"
            >
                <g
                    id="Group_8"
                    dataname="Group 8"
                    transform="translate(-320.5 -428.741)"
                >
                    <line
                        id="Line_1"
                        dataname="Line 1"
                        x2="46"
                        transform="translate(320.5 432.5)"
                        fill="none"
                        stroke="#4FA9D2"
                        strokeWidth="1"
                    />
                    <path
                        ...
                        dataname="Icon ionic-ios-arrow-forward"
                        d="M14.249,9.954,11.4,7.111a.535.535,0,0,1,0-.759.542.542,0,0,1,.761,0l3.223,3.221a.536.536,0,0,1,.016.741l-3.236,3.243a.537.537,0,1,1-.761-.759Z"
                        transform="translate(352.004 422.545)"
                        fill="#4FA9D2"
                    />
                </g>
            </svg>
    ...
    return (
        <div className="pb-5 border-b border-solid mb-5">
            ...
            >
                {options.map((item, index, id) => {
                    ...
                    if (appliedFilters.hasOwnProperty(attribute.code)) {
                        if (
                            appliedFilters[attribute.code].includes(
                                item.id.toString()
                            )
                        ) {
                            checked = true;
            ...
            {options.length > 4 ? (
                ...
                >
                    {showMore
                        ? __("client.filter_less", localizations)
                        : __("client.filter_more", localizations)}{" "}
                    <BiChevronDown
    ...
    const increase = () => {
        ...
        if (props.cart)
            if (props.count >= number + 1) {
                ...
                setNumber(number);
                toast.warn(
                    __("client.remaining_only", localizations) +
                        " " +
                        props.count
                );
            }
...
export const CartItem = (props) => {
    ...
    function addToCart(product, qty) {
        if (props.count >= qty) {
            ...
            toast.warn(
                __("client.remaining_only", localizations) + " " + props.count
            );
        }
    }
                ...
                <button
                    onClick={() => {
                        ...
                        if (props.cart) {
                            //alert(4);
                            removeItem(props.id);
                        } else removeFromWishlist(props.id);
                    }}
                >
                    {/*<Delete />*/}
                    <svg
                        ...
                        width="18.797"
                        height="21.93"
                        viewBox="0 0 18.797 21.93"
                    >
                        <g id="delete" transform="translate(-4.5 -2.25)">
                            <path
                                id="Path_93"
                                dataname="Path 93"
                                d="M13.5,13.5h1.566v9.4H13.5Z"
                                transform="translate(-2.734 -3.418)"
                                fill="#434343"
                            />
                            <path
                                id="Path_94"
                                dataname="Path 94"
                                d="M20.25,13.5h1.566v9.4H20.25Z"
                                transform="translate(-4.785 -3.418)"
                                fill="#434343"
                            />
                            <path
                                id="Path_95"
                                dataname="Path 95"
                                d="M4.5,6.75V8.316H6.066V23.98a1.566,1.566,0,0,0,1.566,1.566H20.164A1.566,1.566,0,0,0,21.73,23.98V8.316H23.3V6.75ZM7.633,23.98V8.316H20.164V23.98Z"
                            ...
                            <path
                                id="Path_96"
                                dataname="Path 96"
                                d="M13.5,2.25h6.266V3.816H13.5Z"

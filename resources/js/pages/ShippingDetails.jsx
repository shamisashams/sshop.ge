...
const ShippingDetails = ({seo}) => {
  ...
  const [city, setCity] = useState(selected ? selected : __('client.choose_city',localizations));
  const [shipPrice, setShipPrice] = useState(shipping ? shipping.ship_price : 0);

    const [selectedCity, setSelectedCity] = useState(
        shipping ? shipping.city_id : null
    );
  ...
  const handleClick = (item) => {
    ...
    setChooseCity(false);
      ...
      setShipPrice(item.ship_price);
      values.city_id = item.id;
